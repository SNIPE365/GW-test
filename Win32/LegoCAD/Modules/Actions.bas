...
sub File_Open()
   ...
   do
      with tOpen
         ...
         .lpstrFilter = @ _
            !"Supported Files (LDR)\0*.ldr;*.mpd\0" _            
         ...
         .lpstrTitle = NULL
         .lpstrDefExt = @"ls"
         .Flags = OFN_FILEMUSTEXIST or OFN_PATHMUSTEXIST or OFN_NOCHANGEDIR or OFN_EXPLORER 'OFN_ALLOWMULTISELECT
...
sub Code_ToggleSidePanel()
  ...
  if g_tCfg.bShowSidePanel  then iToggledState or= MFS_CHECKED  
  Menu.MenuState( g_hCurMenu,g_CurItemID,iToggledState )  
end sub
